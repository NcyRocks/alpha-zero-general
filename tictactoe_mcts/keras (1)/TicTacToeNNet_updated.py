...
class TransformerBlock(layers.Layer):
    def __init__(self, embed_dim, num_heads, ff_dim, rate=0.1, output_dim = 9): # hardcode out_DIM
...
class TokenAndPositionEmbedding(layers.Layer):
    ...
    def call(self, x):
        ...
        x = self.board_emb(x)
        x = tf.reshape(x, shape = (-1, 9, 64)) ## HArdcoded 16/9 here depending on board size.
